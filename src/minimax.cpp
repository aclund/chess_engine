...
Search minimax( Chess_Board chess_board, Move_Tree *head, Hash history, int depth, int alpha, int beta, bool maxer ) {
	...
	if( maxer ) {
		...
		for( int n = 0; n < head->moves_arr.size(); n++ ) {
			...
			Search search = minimax( chess_moved, head->moves_arr[n].children, track, depth-1,
						 alpha, beta, false );
